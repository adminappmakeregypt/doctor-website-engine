...
export function setupBooking(db, clinicId, settings, site) {
  ctx = { db, clinicId, settings, slug: site.slug, siteDoctorId: site.doctorId || "", doctorId: site.doctorId || "" };
  if (document.body.dataset.bookingBound) return;
  document.body.dataset.bookingBound = "1";
  // every booking button on the page (header, hero, hours card, bottom) opens the form
  document.addEventListener("click", (e) => {
    const btn = e.target.closest && e.target.closest('#bookBtn, a[href="#book"]');
    if (!btn || !ctx || !bookingEnabled(ctx.settings, ctx)) return;
    e.preventDefault();
    open();
  });
}
...
let raw = null; // { pa, locks } fetched once per open

async function loadAvailability() {
  const { db, clinicId } = ctx;
  ...
  raw = { pa: pa && pa.exists() ? (pa.data() || {}) : {}, locks };
  // a site linked to one doctor books that doctor; otherwise the visitor picks a clinic doctor
  const d = raw.pa;
  const ids = (d.doctorIds || []).filter((id) => (d.sched || {})[id]);
  ctx.choices = ctx.siteDoctorId ? [] :
    ids.map((id) => ({ id, name: ((d.doctors || []).find((x) => x && x.id === id) || {}).name || id }));
  if (!ctx.siteDoctorId && !ctx.choices.some((c) => c.id === ctx.doctorId)) ctx.doctorId = ctx.choices.length ? ctx.choices[0].id : "";
  compute();
}
...
function compute() {
  ...
  if (doctorId) {
    ...
    if (raw.locks) raw.locks.forEach((l) => {
      const x = l.data() || {};
...
function renderBody() {
  ...
  const pick = (ctx.choices || []).length > 1
    ? '<label class="bk-label" for="bkDoc">الطبيب</label><select id="bkDoc" class="bk-select">' +
      ctx.choices.map((c) => '<option value="' + esc(c.id) + '"' + (c.id === ctx.doctorId ? " selected" : "") + ">" + esc(c.name) + "</option>").join("") +
      "</select>"
    : "";
  body.innerHTML = pick +
    '<div class="bk-grid">' +
      ...
      '<div><label class="bk-label" for="bkPhone">رقم الموبايل <small>(نتعرّف عليك به في الزيارات القادمة)</small></label><input id="bkPhone" type="tel" maxlength="16" autocomplete="tel" inputmode="tel" placeholder="01xxxxxxxxx" dir="ltr" /></div>' +
    "</div>" +
  ...
  renderPicker(dates);
  const sel = document.getElementById("bkDoc");
  if (sel) sel.addEventListener("change", () => { ctx.doctorId = sel.value; compute(); picked.time = ""; renderPicker(); });
}
...
async function submit() {
  const s = ctx.settings || {};
  if (!bookingEnabled(s, ctx) || !ctx.doctorId) return showErr(MSG.disabled);
