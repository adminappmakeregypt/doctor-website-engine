...
import {
  doc, getDoc, setDoc, deleteDoc, collection, getDocs, query, where, limit, arrayUnion
...
$("edSave").addEventListener("click", async () => {
  ...
  try {
    ...
    if (newSlug !== oldSlug) {
      let doctorEmail = String(editing.settings.email || "").toLowerCase(), ownerUid = "";
      try {
        if (oldSlug) {
          const od = await getDoc(doc(db, "websiteDirectory", oldSlug));
          if (od.exists() && od.data().doctorEmail) doctorEmail = od.data().doctorEmail;
          if (od.exists() && od.data().ownerUid) ownerUid = od.data().ownerUid;
        }
      } catch (e) { /* ignore */ }
      await setDoc(doc(db, "websiteDirectory", newSlug),
        { clinicId: editing.clinicId, slug: newSlug, doctorEmail, ownerUid, updatedAt: Date.now() }, { merge: true });
