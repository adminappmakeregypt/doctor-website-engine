...
function state(which) {
  ["stLoading", "stNotFound", "stDisabled"].forEach((id) => hide($(id)));
  hide($("site"));
...
function renderHero(s) {
  ...
  const ph = $("hPhone");
  if (has(s.phone)) { ph.href = "tel:" + String(s.phone).replace(/\s/g, ""); show(ph); } else hide(ph);
...
async function boot() {
  const slug = readSlug();
  if (!slug) { state("stNotFound"); return; }

  const clinicId = await resolveClinicId(slug);
  if (!clinicId) { state("stNotFound"); return; }
