...
- الصور تُرفع إلى Firebase Storage على المسار `clinics/{clinicId}/website/{profile|cover|logo}/...`.
- كل حفظ ينعكس على الموقع العام مباشرة (onSnapshot) دون إعادة نشر.

### قاعدة أمان التخزين (Storage Rules)
```
match /clinics/{clinicId}/website/{allPaths=**} {
  allow read: if true;
  allow write: if request.auth != null;
}
```
