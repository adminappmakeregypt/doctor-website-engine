...
## إصلاح v24 — تثبيت جلسة الطبيب

- عادت لوحة الطبيب إلى جلسة Firebase الأصلية المستقرة مع تفعيل الحفظ المحلي صراحةً.
- بقيت لوحة المدير في جلسة مستقلة، ولا تنفذ تسجيل خروج عند رفض صلاحية حساب طبيب.
- ارفع `firebase-config.js` و`doctor.js` و`doctor.html` و`admin.js` و`admin.html` معًا، وأغلق كل التبويبات القديمة قبل التجربة.

## إصلاح v22 — الخروج التلقائي بعد تسجيل الدخول

- أصبحت جلسة `admin.html` مستقلة عن جلسة `doctor.html` داخل المتصفح نفسه.
- لم تعد لوحة المدير تسجل خروج حساب الطبيب من جميع الصفحات عند رفض صلاحية المدير.
- ارفع ملفات `firebase-config.js` و`admin.js` و`admin.html` و`doctor.js` و`doctor.html` معًا، ثم حدّث الصفحة تحديثًا قويًا.
...
## إصلاح v25 — الملكية بمعرّف الحساب (UID)
- ملكية الموقع أصبحت بمعرّف حساب Firebase (uid) بدل البريد: `websiteDirectory/{slug}.ownerUid` و`clinics/{clinicId}/websiteSettings/main.ownerUids`.
- لوحة الطبيب تبحث عن موقعه بـ uid أولاً، ثم users/{uid}، ثم البريد للمواقع القديمة فقط.
- الترحيل تلقائي: عند فتح لوحة المدير تُربط المواقع القديمة بـ uid صاحب البريد إن وُجد له ملف في users.
- القواعد: صاحب الـ uid يعدّل بيانات موقعه وصوره فقط، ولا يستطيع تعديل قائمة الملاك.
- أضيف حقل «آراء المرضى» في تبويب النبذة.
- الحجز الأونلاين وربطه ببرنامج العيادة بدون تغيير (v21).
