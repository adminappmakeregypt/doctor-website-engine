...
import { doctorAuth as auth, doctorDb as db, doctorStorage as storage } from "./firebase-config.js?v=23";
import {
...
async function resolveClinicId(user) {
  ...
  if (profile) {
    ...
    if (profile.clinicId) return String(profile.clinicId);
  }
  // fallback: directory entry that carries the doctor's email
  try {
